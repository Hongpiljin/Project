...
- 문제 : 게시글 신고 승인 후, 삭제 대신 어떻게 목록에서 숨김 처리할지 고민  
  해결 : 게시글 상태 값을 0(미처리)에서 1(처리완료)로 변경하여 숨김 처리 구현

- 문제 : 카테고리 변경 및 검색 후 페이징 처리가 되지 않아 게시물이 과다 출력됨  
  해결 : 페이징 로직을 적용하여 검색 결과에 맞는 페이징 기능 구현

- 문제 : 페이징 처리 코드를 재사용하지 못해, 비슷한 기능 개발에 시간이 많이 소요됨  
  해결 : 공통 페이징 모듈 설계 필요성을 인지하고 이후 개선 계획 수립
